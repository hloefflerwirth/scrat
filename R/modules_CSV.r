modules.CSV.sheets <- function(env, spot.list, main, path)
{
  ...
  for (m in seq_along(spot.list$spots))
  {
    ...
    if(typeof(env$indata)[1] == "S4")
    {
      e.max <- apply(env$indata@assays$RNA@counts[spot.list$spots[[m]]$genes, ,drop=FALSE], 1, max)
      e.min <- apply(env$indata@assays$RNA@counts[spot.list$spots[[m]]$genes, ,drop=FALSE], 1, min)
    }
    else 
    {
    ...
    if(typeof(env$indata)[1] == "S4")
    {
      out <- cbind(out,
                   ...
                   "SD"=apply(env$indata@assays$RNA@counts[o, ,drop=FALSE], 1, sd),
                   "max delta e"=apply(env$indata@assays$RNA@counts[o, ,drop=FALSE], 1, max),
                   "min delta e"=apply(env$indata@assays$RNA@counts[o, ,drop=FALSE], 1, min),
