pipeline.detectSpotsIntegral <- function(env)
{
  ...
  for (i in seq_along(sample.spot.list))
  {
    ...
    {
      env$spot.list.overexpression$overview.mask[spot.metagenes] <- i
  ...
  for (i in 1:n.cluster)
  {
    ...
    if (length(geneset.genes) > 0)
    {
      ...
      env$spot.list.kmeans$spots[[env$LETTERS[i]]]$beta.statistic <- get.beta.statistic(set.data=env$metadata[env$spot.list.kmeans$spots[[env$LETTERS[i]]]$metagenes,,drop=FALSE],
