pipeline.prepareAnnotation <- function(env)
{
  ...
  env$unique.protein.ids <- unique(env$gene.info$ids)
  env$unique.protein.ids <- env$unique.protein.ids[which(env$unique.protein.ids!="")]
  
  suppressWarnings({  biomart.table <- getBM(c("ensembl_gene_id", "go_id", "name_1006", "namespace_1003"), "ensembl_gene_id", env$unique.protein.ids, mart, checkFilters=FALSE) })
  ...
  data(scrat.genesets)
  env$gs.def.list  <- c(env$gs.def.list, scrat.genesets)

  env$gs.def.list <- lapply(env$gs.def.list, function(x) {
    x$Genes <- intersect(x$Genes, env$unique.protein.ids)
    return(x)
  })


  env$gs.def.list <- env$gs.def.list[ which(sapply(sapply(env$gs.def.list, head, 1), length) >= 2) ]
